...
import pytz
from form import TaskForm, LoginForm, RegistrationForm, UserForm, UpdateProfileForm
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if existing_user :
            flash("Email already registered. Pleas use a different email." , "danger")
            return render_template("register.html", form=form)
        
        # 사용자 이름 중복 확인
        existing_user_username = User.query.filter_by(username=form.username.data).first()
        if existing_user_username :
            flash("사용자 이름 중복","danger")
            return render_template("register.html", form=form)
...
@app.route("/admin/delte/<int:user_id>")
