...
def admin_analysis():
    ...
    for user in users:
        ...
        tasks_overdue = sum(
            1
            for task in tasks
            if (task.completion_date and task.due_date and task.completion_date > task.due_date) or
               (not task.completion_date and task.due_date and task.due_date < current_date)
        )
